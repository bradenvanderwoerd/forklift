...
class NavigationController:
    ...
    def navigate(self) -> bool:
        ...
        if not self.is_centered(target_x_cam):
            ...
            if turn_speed > 0: # Positive turn_speed (marker to left, PID wants to turn left)
                # If it's turning wrong way, swap these:
                self.motor_controller.turn_right(abs(turn_speed)) # Swapped: was turn_left
            else: # Negative turn_speed (marker to right, PID wants to turn right)
                # If it's turning wrong way, swap these:
                self.motor_controller.turn_left(abs(turn_speed))  # Swapped: was turn_right
